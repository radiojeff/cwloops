...
*  The `--fs` is the Farnsworth speed (not words per minute) -- this also goes
by the name *character speed*.  In other words the speed of each character can be
timed to a different rate than the rate of all of the symbols (words per minute).  
You can set the Farnsworth speed (character speed) to be the same words per minute if you like.
...
where slight perturbations can be heard by an audiophile.   Above 192kbit
it becomes difficult for the average person to truly hear the difference.  Stick
with 192kbit.
...
W1ABC W7DEF FB ALICE TNX BT NAME BOB ES QTH NR SEATTLE SEATTLE WA BT UR 5NN HR BT ANT LOOP UP 30 M ES RIG K3 ES KPA BKU W1ABC DE W7DEF K
...
$ echo "W1ABC W7DEF FB ALICE TNX BT NAME BOB ES QTH NR SEATTLE SEATTLE WA BT UR 5NN HR BT ANT LOOP UP 30 M ES RIG K3 ES KPA BKU W1ABC DE W7DEF K" | python3 play.py -f 668 --fs 24 --wpm 22 -o bob-sig.wav
