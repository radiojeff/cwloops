...
def morseSampleDuration(code, sps, wpm, fs=None):
  ...
  dotArr = np.ones(samplesPerDot, dtype=bool)
  dashArr = np.ones(samplesPerDash, dtype=bool)
  eGapArr = np.zeros(samplesBetweenElements, dtype=bool)
  cGapArr = np.zeros(samplesBetweenLetters, dtype=bool)
  wGapArr = np.zeros(samplesBetweenWords, dtype=bool)
